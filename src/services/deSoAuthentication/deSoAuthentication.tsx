...
export const authenticateWithDeSoIdentity = async (): Promise<boolean> => {
    ...
    if (response.type === 'success') {
        try {
            const derivedAuthentication: DerivedAuthentication = response.params as unknown as DerivedAuthentication;
            ...
            const appendExtraDataResponse = await deSoApi.appendExtraDataToTransaction(
                authorizationResponse.TransactionHex as string,
                derivedAuthentication.compressedDerivedPublicKey
            );

            const signedTransaction = signing.signTransaction(appendExtraDataResponse.TransactionHex as string, derivedAuthentication.derivedSeedHex);

            await deSoApi.submitTransaction(signedTransaction);
            await wait(3000);
            const derivedKeys = await deSoApi.getUsersDerivedKeys(derivedAuthentication.publicKey);

            if (derivedKeys.DerivedKeys[derivedAuthentication.derivedPublicKey]?.IsValid) {
                SecureStore.setItemAsync(Constants.SECURE_STORAGE_DESO_CONNECTION, JSON.stringify(derivedAuthentication));
                return true;
            }
