...
export function ChatListScreen(props: Props): JSX.Element {
    ...
    const renderRightCancelSwipe = (dragX: Animated.AnimatedInterpolation, index: number): JSX.Element | undefined => {
    ...
    const keyExtractor = (item: Contact, index: number) => item.username + String(index);
    const renderItem = ({ item, index }: { item: Contact, index: number }) => <Swipeable
        renderRightActions=
        {
            (_progress: Animated.AnimatedInterpolation, dragX: Animated.AnimatedInterpolation) => renderRightCancelSwipe(dragX, index)
        }>
        <TouchableOpacity
            activeOpacity={0.7}
            onPress={() => props.navigation.push(
                NavigationElement.ChatScreen, { contact: JSON.stringify(item) }
            )}
        >
