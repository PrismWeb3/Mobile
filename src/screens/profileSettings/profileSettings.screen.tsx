...
export function ProfileSettingsScreen(props: Props): JSX.Element {
    ...
    const settings: Setting[] = [
        ...
        {
            icon: <MaterialIcons name='important-devices' size={23} color={iconColor} />,
            text: 'Add Device',
            action: () => props.navigation.push(NavigationElement.AddDeviceScreen)
        },
        {
            icon: <MaterialCommunityIcons name='account-multiple-plus' size={23} color={iconColor} />,
            text: 'Accounts',
            action: () => props.navigation.push(NavigationElement.AccountsScreen)
        }
    ];

    const renderItem = ({ item }: { item: Setting }) => <TouchableOpacity
        style={[styles.settingContainer]}
        activeOpacity={0.7}
        onPress={item.action}
    >
        {item.icon}
        <Text style={[styles.settingText, globalStyles.fontColorPrimary]}>{item.text}</Text>

        <Ionicons name='chevron-forward' size={18} color={globalStyles.fontColorSecondary.color} />
    </TouchableOpacity>;
    ...
    return <View style={[styles.container, globalStyles.containerColorMain]}>
        <DefaultScreenHeader title={'Settings'} back={true} navigation={props.navigation} />
