...
import { ActivityIndicator, NativeScrollEvent, SectionList, StyleSheet, Text, TouchableOpacity } from 'react-native';
...
export function ChatMessages(props: Props): JSX.Element {
    ...
    const updateSections = (messages: Message[], newMessage = false) => {
        ...
        for (const key of keys) {
            ...
            const sectionMessages = existedSection ? existedSection.data : newSections[newSections.length - 1].data;

            if (sectionMessages.length > 0) {
                sectionMessages[0].lastOfGroup = true;
                for (let i = 1; i < sectionMessages.length; i++) {
                    sectionMessages[i].lastOfGroup = sectionMessages[i].isSender !== sectionMessages[i - 1].isSender;
                }
            }
        }
    ...
    function groupMessagesByDay(messages: Message[]): { [key: string]: Message[] } {
    ...
    const goToReply = (message: Message) => {
        ...
        try {
            for (let i = 0; i < sections.length; i++) {
                ...
                // must use message ID
                const messageIndex = section.data.findIndex(m => m.text === message.text);

                if (messageIndex !== -1) {
                    sectionListRef.current.scrollToLocation({ itemIndex: messageIndex, sectionIndex: i, animated: true, viewPosition: 0.5 });
                    break;
                }
            }
        } catch { }
    ...
    const renderSectionFooter = ({ section }: { section: MessagesSection }): JSX.Element =>
    ...
    return <>
...

        {
            ...
            <TouchableOpacity
                ...
                <AntDesign name='downcircleo' size={24} color={globalStyles.fontColorAccent.color} />
            </TouchableOpacity>
        }
    </>;
}
