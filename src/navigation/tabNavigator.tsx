...
function TabBar({ props }: { props: BottomTabBarProps }): JSX.Element {
    ...
    const getTabElement = (route: string): JSX.Element => {
        ...
        switch (route) {
            ...
            case NavigationElement.ChatStack:
                icon = <Ionicons name='chatbubbles-sharp' size={28} color={iconColor} />;
                break;
            case NavigationElement.ProfileStack:
                icon = <MaterialCommunityIcons name='account-circle-outline' size={28} color={iconColor} />;
                break;
        }
    ...
    return <SafeAreaView style={globalStyles.containerColorSecondary}>
        <View style={styles.tabsContainer}>
            {
                routes.map(p_route =>
                    getTabElement(p_route)
                )
            }
...
export function TabNavigator() {
    return (
        ...
        >
            ...
            <Tab.Screen name={NavigationElement.ProfileStack} component={ProfileStack} />
        </Tab.Navigator>
    );
}
